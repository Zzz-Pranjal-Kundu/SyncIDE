...
function SnippetsPage() {
  ...
  const [selectedLanguage, setSelectedLanguage] = useState<string | null>(null); // this can either be a string or a null
  const [view, setView] = useState<"grid" | "list">("grid"); //set the type of view(grid/list), by default- grid view
  ...
  //A set of all the languages in which snippets exist
  const languages = [...new Set(snippets.map((s) => s.language))];
  //Get top 5 languages
  const popularLanguages = languages.slice(0, 5);
  //To get all the search results based on the typed word in the search box
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-12">
            ...
            <div className="flex flex-wrap items-center gap-4">
                ...
                <button
                    key={lang}
                    // {/* if the language is clicked twice, deselect it */}
            ...
            className={`grid gap-6 ${
                view === "grid"
                ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3"  // Grid View 
                : "grid-cols-1 max-w-3xl mx-auto"  // List view
            }`}
            layout
            >
