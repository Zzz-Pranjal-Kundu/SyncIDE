...
function LanguageSelector({hasAccess}: { hasAccess: boolean}) {
  ...
  const currentLanguageObject  = LANGUAGE_CONFIG[language] //to get the current selected language object and its default code and configurations from LANGUAGE_CONFIG file in _constants

  // close the dropdown whenever clicked outside the dropdown
  useEffect(() => {
  ...
  // //HYDRATION FIX
  // useEffect(() => {
  //   setMounted(true);  // if we are in the client then return true, else null
  // }, []);
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        ${!hasAccess && language !== "javascript" ? "opacity-50 cursor-not-allowed" : ""}`}  // Cursor not allowed if user does not have the access
      >
        {/* Decoration */}
        <div
