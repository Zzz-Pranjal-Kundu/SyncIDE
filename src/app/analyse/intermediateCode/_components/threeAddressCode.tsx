...
export function generate3AC(code: string): Quad[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("int ") || line.startsWith("float ") || line.startsWith("string ") || line.startsWith("char ")) {
      const [_, rest] = line.replace(";", "").split(/int |float |string |char /);
      if (rest.includes("=")) {
        const [name, value] = rest.split("=");
        const rhsTrimmed = value.trim();
        const ops = ["+", "-", "*", "/"];
        const opFound = ops.find(op => rhsTrimmed.includes(op));
        if (opFound) {
          const [a, b] = rhsTrimmed.split(opFound);
          const t = newTemp();
          result.push({ op: opFound, arg1: a.trim(), arg2: b.trim(), result: t });
          result.push({ op: "=", arg1: t, arg2: "", result: name.trim() });
        } else {
          result.push({ op: "=", arg1: rhsTrimmed, arg2: "", result: name.trim() });
        }
      }
      continue;
    }
